...
fn run() !void {
    ...
    try server.addTool(.{
        .name = "list_chats",
        .description = "List all WhatsApp chats/conversations. Returns chat name, last message, unread count, and last message date. Use 'limit' parameter to control number of results (default 20).",
        .handler = &listChatsHandler,
    });

    try server.addTool(.{
        .name = "get_chat_messages",
        .description = "Get messages from a specific WhatsApp chat. Requires 'chat_name' (partial name match) or 'chat_jid' (exact JID). Optional: 'limit' (default 50), 'offset' for pagination.",
        .handler = &getChatMessagesHandler,
    });

    try server.addTool(.{
        .name = "search_messages",
        .description = "Search for messages containing specific text across all chats. Requires 'query' parameter. Optional: 'limit' (default 50).",
        .handler = &searchMessagesHandler,
    });

    try server.addTool(.{
        .name = "get_chat_stats",
        .description = "Get statistics for a specific chat including message count, date range, and participant info. Requires 'chat_name' or 'chat_jid'.",
        .handler = &getChatStatsHandler,
    });

    try server.addTool(.{
        .name = "list_groups",
        .description = "List all WhatsApp group chats with member count and creation info. Optional: 'limit' (default 20).",
        .handler = &listGroupsHandler,
    });

    try server.addTool(.{
        .name = "get_group_members",
        .description = "Get members of a specific WhatsApp group. Requires 'group_name' (partial match) or 'group_jid'.",
